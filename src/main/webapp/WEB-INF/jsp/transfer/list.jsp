				...
				<div class="layui-card">
					...
					<div class="layui-card-header">
						<button class="layui-btn"
							onclick="xadmin.open('新建调职申请单','addInput',800,600)">
							<i class="layui-icon"></i>新建调职申请单
						</button>
					</div>
						...
						<table class="layui-table layui-form">
							...
							<tbody>
								<c:forEach items="${transfers }" var="transfers">
									<tr>
										<td>${transfers.id }</td>
										<td>${transfers.applyname }</td>
										<td>${transfers.applyDate }</td>
										<td>${transfers.currentDept }</td>
										<td>${transfers.currentPosition }</td>
										<td>${transfers.currentLevel }</td>
										<td>${transfers.hopeDate }</td>
										<td>${transfers.targetDept }</td>
										<td>${transfers.tatgetPosition }</td>
										<td>${transfers.targetLevel }</td>
										<td class="td-manage"><a title="编辑"
											onclick="xadmin.open('编辑','updateInput?id=${transfers.id}',800,600)"
											href="javascript:;"> <i class="layui-icon">&#xe642;</i>
										</a> <a title="删除" onclick="member_del(this,${transfers.id })"
											href="javascript:;"> <i class="layui-icon">&#xe640;</i>
										</a></td>
									</tr>
								</c:forEach>
