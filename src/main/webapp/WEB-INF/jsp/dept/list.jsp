							...
							<tbody>
								<c:forEach items="${depts }" var="dept">
									<tr>
										...
										<td>${dept.name }</td>
										<td><c:choose>
												<c:when test="${empty dept.desc }">该部门比较懒没有介绍</c:when>
												<c:otherwise>${ dept.desc }</c:otherwise>
											</c:choose></td>

										<td class="td-manage"><a title="编辑"
											onclick="xadmin.open('编辑','updateInput?id=${dept.id}',800,600)"
											href="javascript:;"> <i class="layui-icon">&#xe642;</i>
										</a> <a title="删除" onclick="member_del(this,${dept.id })"
											href="delete/${dept.id }"> <i class="layui-icon">&#xe640;</i>
										</a></td>
									</tr>
								</c:forEach>
							</tbody>
